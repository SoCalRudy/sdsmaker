...
const widthContent = store.getState().pdf.widthContent;
const marginPositionRight = store.getState().pdf.marginPositionRight;

// gets the height of text for single lines and multiple lines wrapping to conform to max width
export const getTextHeight = (
  doc: jsPDF,
  text: string,
  maxWidth: number = widthContent
) => {
...
export const getXRightAlign = (
  doc: jsPDF,
  text: string,
  xPositionEnd: number = marginPositionRight
) => {
    console.log(xPositionEnd)
...
export const updateYFromText = (doc: jsPDF, text: string, extraSpace?: number) => {
